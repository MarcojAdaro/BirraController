...
prueba1=1; // pongo en 1 prueba1 para avisar al LCD que esta en modo FER
  if (temp1>=20){//------------------------------> cambiar temperatura minima modo fermentador 1<------ACA CAMBIO TEMPERATURA
    fermentar(n_ferm,Relemotor,Relepin);
  }
...
/*
void fermentar(boolean n_ferm ,int Motor, int Pin, float tem){


prueba1=1; // pongo en 1 prueba1 para avisar al LCD que esta en modo FER
  if (temp>=20){//------------------------------> cambiar temperatura minima modo fermentador 1<------ACA CAMBIO TEMPERATURA
    apagar[n_ferm]=false;
    digitalWrite(Motor, HIGH);  //Enciendo el rele del motor 
    digitalWrite(Pin, HIGH);   // Enciendo rele de la electrovalvula 1
  }else if(temp<18){
     digitalWrite(Pin, LOW);   //  Apago Electrovalvula
     apagar[n_ferm]=true;
  }
